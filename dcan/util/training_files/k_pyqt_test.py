...
import psutil
import argparse
from k_test1 import Ui_MainWindow
...
class Window(Ui_MainWindow):
    ...
    def run_program(self):
        ...
        if self.pushButton.text() == "run":
            ...
            if any(inp.text() == "" for inp in self.inputDict.values()):
                self.show_popup("Please fill out all input fields")
        ...
        # If process is currently running
            ...
            self.temp_thread.stop() # Stops subprocesses within thread. This will cause the finish signal to be sent
            self.pushButton.setText('run')
            
    def populate_inputs(self, preset=None):
        try:
            preset_file = preset if preset else self.line_set_preset.text()
            with open(f"{self.script_dir}/automation_presets/{preset_file}") as f:
    ...
    def save_preset(self):
        ...
        if not os.path.isfile(f"{self.script_dir}/automation_presets/{self.line_save_preset.text()}.config"):
            with open(f"{self.script_dir}/automation_presets/{self.line_save_preset.text()}.config", "w") as f:
    ...
    def show_popup(self,error):
        ...
        msg_box.setText(f"Error: {error}")
        msg_box.setWindowTitle("Message Box")
        msg_box.setStyleSheet("background-color: rgb(177,156,217)")

        # Show the message box and get the user's response
        ret = msg_box.exec_()
        
def main(): 
    parser = argparse.ArgumentParser(description='Load preset for automation.')
    parser.add_argument('--preset', type=str, help='The name of the preset to load.')
    ...
    # Populate inputs with the provided preset if any
    MainWindow.show()
    if args.preset:
        ui.populate_inputs(args.preset)
    sys.exit(app.exec_())
